...
interface CustomVideoConferenceProps {
  ...
  userId?: number;
  userToken?: string;
  jwtToken?: string; // 🎯 添加Token用于API认证
  hostUserId?: number;
}
  ...
  // 🎯 当房间连接时，获取房间详情
    ...
    const fetchRoomDetails = async () => {
      try {
        ...
        const parsedMaxSlots = Number(payload.max_mic_slots);
        setRoomDetails(prev => {
          const next = { ...(prev ?? {}) };
          if (Number.isFinite(parsedMaxSlots)) {
            next.maxMicSlots = parsedMaxSlots;
          }
          if (typeof payload.room_name === 'string' && payload.room_name) {
            next.roomName = payload.room_name;
          }
          if (typeof payload.room_state === 'number') {
            next.roomState = payload.room_state;
          }
          return Object.keys(next).length > 0 ? next : prev;
        });
        if (payload.chat_global_mute !== undefined) {
          const muteValue = payload.chat_global_mute;
